...
pub async fn login(args: LoginArgs) -> Result<()> {
    ...
    let tenant_config = if let Some(name) = tenant_name {
        ...
        match config_manager.get_tenant_or_env(name) {
            Ok(tenant) => {
                println!("{} Loaded tenant: {} ({})", "✓".green(), name.bold(),
                ...
                if args.import_only {
                    println!("\n{} Import complete. Run 'ctl365 login {}' to authenticate.", "→".cyan(), name);
                    return Ok(());
                }
            ...
            Err(_) => {
                return Err(crate::error::Ctl365Error::ConfigError(format!(
                    ...
                    name, name.to_uppercase(), name.to_lowercase()
                )));
            }
        }
    } else if let (Some(tenant_id), Some(client_id)) = (&args.tenant_id, &args.client_id) {
        // Quick setup: Create new tenant config on-the-fly
        println!("\n{} Quick setup mode: Creating tenant configuration...", "→".cyan());

        let name = args.tenant.unwrap_or_else(|| {
            // Generate a friendly tenant name from tenant_id first segment
            let auto_name = tenant_id.split('-').next().unwrap_or("my-tenant").to_string();
            println!("\n{} Auto-generated tenant name: {}", "→".cyan(), auto_name.bold());
            println!("{} You can rename it later with: ctl365 tenant add <new-name> ...", "💡".dimmed());
            auto_name
        });

        let auth_type = if args.client_credentials || args.client_secret.is_some() {
            println!("{} Using client credentials flow (automation mode)", "🔐".cyan());
        ...
        return Err(crate::error::Ctl365Error::InvalidConfig(
            ...
            ctl365 login --tenant-id ID --client-id ID  # Quick setup".into(),
        ));
    };
    ...
    let auth_result = match tenant_config.auth_type {
        AuthType::DeviceCode => {
            auth.login_device_code(&tenant_config).await
        }
        AuthType::ClientCredentials => {
    ...
    match auth_result {
        Ok(_) => {
            ...
            println!("\n{} Active tenant: {}", "→".cyan(), tenant_config.name.bold());
            Ok(())
        }
        Err(e) => {
